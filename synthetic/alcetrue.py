...
def _first_order_ale_quant(predictor, model_x3, dataset, feature, bins, epsilon3, epsilon4):
    ...
    for offset in range(2):
        ...
        mod_dataset[:,2]=10*sigmoid(-mod_dataset[:,0]-mod_dataset[:,1])-5+epsilon3.flatten()
        mod_dataset=torch.tensor(mod_dataset)
        predictions.append(10*sigmoid(mod_dataset[:,0]+mod_dataset[:,1]-mod_dataset[:,2])+epsilon4.flatten())
